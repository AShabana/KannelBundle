<?php
        ...
        for($i=0; $i<count($Conf); $i++)
        {
                ...
                print  $i+1  . ")) $Conf[$i] [ <a href=http://" . $appConf['global']['portal_host'] . $appConf['global']['portal_port'] . "/get_kanStatus.php?pass=$pass&port=$port> Status </a>]
                                             ...
                                             [ <a href=http://" . $appConf['global']['portal_host'] . $appConf['global']['portal_port'] . "/control_smscs.php?pass=$pass&port=$port> Contole links </a>]
                                             [ <a href=http://" . $appConf['global']['portal_host'] . $appConf['global']['portal_port'] . "/get_kanAggregated.php?pass=$pass&port=$port> Aggregated by status snapshot </a>]
